...
static uchar_t *
ctf_copy_smembers(ctf_dtdef_t *dtd, uint32_t soff, uchar_t *t)
{
...
static uchar_t *
ctf_copy_lmembers(ctf_dtdef_t *dtd, uint32_t soff, uchar_t *t)
{
	ctf_dmdef_t *dmd = ctf_list_next(&dtd->dtd_u.dtu_members);
	ctf_lmember_t ctlm;
...
static uchar_t *
ctf_copy_emembers(ctf_dtdef_t *dtd, uint32_t soff, uchar_t *t)
{
	ctf_dmdef_t *dmd = ctf_list_next(&dtd->dtd_u.dtu_members);
	ctf_enum_t cte;
...
ctf_update(ctf_file_t *fp)
{
	...
	for (dtd = ctf_list_next(&fp->ctf_dtdefs);
		...
		case CTF_K_ARRAY:
			cta.cta_contents = (uint32_t)
			    dtd->dtd_u.dtu_arr.ctr_contents;
			cta.cta_index = (uint32_t)
...
ctf_set_array(ctf_file_t *fp, ctf_id_t type, const ctf_arinfo_t *arp)
{
	...
	if (dtd == NULL || LCTF_INFO_KIND(fp, dtd->dtd_data.ctt_info) != CTF_K_ARRAY)
...
ctf_add_type(ctf_file_t *dst_fp, ctf_file_t *src_fp, ctf_id_t src_type)
{
	...
	if (kind == CTF_K_INTEGER || kind == CTF_K_FLOAT) {
		...
		if (dst_type != CTF_ERR) {
			...
			if (LCTF_INFO_ISROOT(fp, dst_tp->ctt_info) &
			    CTF_ADD_ROOT) {
				/*
